...
public:
    ...
    virtual std::vector<PandaVar>::const_iterator begin() const { throw std::runtime_error("Type is not iterable"); }
...
public:
    using Value = std::variant<
        std::monostate, int64_t, double, bool, std::string, std::shared_ptr<PandaObject>
    >;
    ...
    template <typename K> PandaVar& pandac_getitem(const K& key);
    template <typename K> PandaVar& operator[](const K& key) { return pandac_getitem(key); }
...
public:
    ...
    PandaVar& getItem(const PandaVar& key) override { return data.at(static_cast<size_t>(static_cast<int64_t>(key))); }
    int64_t length() const override { return static_cast<int64_t>(data.size()); }
...
inline PandaVar PandaObject::getProperty(const std::string& name) { throw std::runtime_error("Property '" + name + "' not found in " + getClassName()); }
inline void PandaObject::setProperty(const std::string& name, const PandaVar& value) { throw std::runtime_error("Cannot set property '" + name + "' in " + getClassName()); }
inline PandaVar& PandaObject::getItem(const PandaVar& key) { throw std::runtime_error("Type " + getClassName() + " does not support indexing"); }
...
inline PandaVar::operator std::string() const {
    ...
    if (auto p = std::get_if<std::shared_ptr<PandaObject>>(&data)) { if (*p) return (*p)->toString(); return "None"; }
    return "None";
}
template <typename K> inline PandaVar& PandaVar::pandac_getitem(const K& key) {
...
inline void PandaVar::pandac_setprop(const std::string& name, const PandaVar& val) {
    if (auto obj = std::get_if<std::shared_ptr<PandaObject>>(&data)) { (*obj)->setProperty(name, val); return; }
...
inline PandaVar operator+(const PandaVar& a, const PandaVar& b) {
    if (std::holds_alternative<std::string>(a.data) || std::holds_alternative<std::string>(b.data)) return PandaVar(std::string(a) + std::string(b));
    if (std::holds_alternative<double>(a.data) || std::holds_alternative<double>(b.data)) return PandaVar(static_cast<double>(a) + static_cast<double>(b));
...
inline PandaVar operator/(const PandaVar& a, const PandaVar& b) { return PandaVar(static_cast<double>(a) / static_cast<double>(b)); }

inline std::ostream& operator<<(std::ostream& os, const PandaVar& var) { os << std::string(var); return os; }

template <typename T> std::string to_str(const T &val) { return std::string(PandaVar(val)); }
...
void pandac_print(const T& first, const Args&... args) {
    ...
    if constexpr (sizeof...(args) > 0) { std::cout << " "; pandac_print(args...); } 
    else { std::cout << std::endl; }
}
inline PandaVar input(PandaVar prompt = PandaVar("")) {
    if (std::string(prompt) != "") std::cout << std::string(prompt);
    std::string s; std::getline(std::cin, s); return PandaVar(s);
}
...
inline PandaVar pandac_int64_div(const PandaVar& a, const PandaVar& b) { return PandaVar(static_cast<int64_t>(a) / static_cast<int64_t>(b)); }
inline PandaVar pandac_mod(const PandaVar& a, const PandaVar& b) { return PandaVar(static_cast<int64_t>(a) % static_cast<int64_t>(b)); }
inline PandaVar pandac_pow(const PandaVar& a, const PandaVar& b) { return PandaVar(std::pow(static_cast<double>(a), static_cast<double>(b))); }
...
inline PandaVar pandac_neq(const PandaVar& a, const PandaVar& b) { return PandaVar(!static_cast<bool>(pandac_eq(a, b))); }
...
inline PandaVar pandac_greater(const PandaVar& a, const PandaVar& b) { return PandaVar(!static_cast<bool>(pandac_less_eq(a, b))); }
inline PandaVar pandac_greater_eq(const PandaVar& a, const PandaVar& b) { return PandaVar(!static_cast<bool>(pandac_less(a, b))); }
...
inline PandaVar& pandac_assign(PandaVar& lhs, const PandaVar& rhs) { lhs = rhs; return lhs; }
inline PandaVar& pandac_assign_add(PandaVar& lhs, const PandaVar& rhs) { lhs = pandac_add(lhs, rhs); return lhs; }
inline PandaVar& pandac_assign_sub(PandaVar& lhs, const PandaVar& rhs) { lhs = pandac_sub(lhs, rhs); return lhs; }
inline PandaVar& pandac_assign_mul(PandaVar& lhs, const PandaVar& rhs) { lhs = pandac_mul(lhs, rhs); return lhs; }
inline PandaVar& pandac_assign_div(PandaVar& lhs, const PandaVar& rhs) { lhs = pandac_div(lhs, rhs); return lhs; }
inline PandaVar& pandac_assign_int_div(PandaVar& lhs, const PandaVar& rhs) { lhs = pandac_int64_div(lhs, rhs); return lhs; }
inline PandaVar& pandac_assign_mod(PandaVar& lhs, const PandaVar& rhs) { lhs = pandac_mod(lhs, rhs); return lhs; }
